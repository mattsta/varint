...
int varintAdaptiveCheckSorted(const uint64_t *values, size_t count) {
    if (count <= 1) return 1; /* Single element is sorted */
...
const char *varintAdaptiveEncodingName(varintAdaptiveEncodingType type) {
    switch (type) {
        case VARINT_ADAPTIVE_DELTA:  return "DELTA";
        case VARINT_ADAPTIVE_FOR:    return "FOR";
        case VARINT_ADAPTIVE_PFOR:   return "PFOR";
        case VARINT_ADAPTIVE_DICT:   return "DICT";
        case VARINT_ADAPTIVE_BITMAP: return "BITMAP";
        case VARINT_ADAPTIVE_TAGGED: return "TAGGED";
        case VARINT_ADAPTIVE_GROUP:  return "GROUP";
