...
void printStats(const char *name, const varintAdaptiveDataStats *stats) {
    ...
    printf("Range:         %lu - %lu (range: %lu)\n",
           stats->minValue, stats->maxValue, stats->range);
    printf("Unique:        %zu (%.1f%%)\n",
           stats->uniqueCount, stats->uniqueRatio * 100);
    printf("Sorted:        %s\n",
           stats->isSorted ? "Yes" : stats->isReverseSorted ? "Reverse" : "No");
    printf("Avg Delta:     %lu\n", stats->avgDelta);
    printf("Max Delta:     %lu\n", stats->maxDelta);
    printf("Outliers:      %zu (%.1f%%)\n",
           stats->outlierCount, stats->outlierRatio * 100);
...
void example_timestamps() {
    ...
    uint64_t timestamps[] = {
        1700000000, 1700000001, 1700000002, 1700000004,
        1700000005, 1700000007, 1700000008, 1700000010,
        1700000012, 1700000013, 1700000015, 1700000017,
        1700000019, 1700000020, 1700000022, 1700000024,
        1700000026, 1700000028, 1700000030, 1700000031,
    };
    ...
    size_t encodedSize = varintAdaptiveEncode(encoded, timestamps, count, &meta);
...
void example_status_codes() {
    ...
    uint64_t statusCodes[] = {
        200, 200, 200, 200, 404, 200, 200, 500, 200, 200,
        200, 200, 304, 200, 200, 200, 200, 200, 404, 200,
        200, 200, 200, 200, 200, 503, 200, 200, 200, 200,
        200, 200, 200, 200, 200, 200, 304, 200, 200, 200,
        200, 200, 200, 404, 200, 200, 200, 200, 200, 200,
    };
    ...
    uint8_t *encoded = malloc(varintAdaptiveMaxSize(count));
    varintAdaptiveMeta meta;
    size_t encodedSize = varintAdaptiveEncode(encoded, statusCodes, count, &meta);
...
void example_user_ids() {
    ...
    uint64_t userIds[] = {
        500000, 500001, 500003, 500005, 500007, 500010,
        500012, 500015, 500018, 500020, 500023, 500025,
        500028, 500030, 500033, 500036, 500038, 500040,
        500043, 500045, 500048, 500050, 500053, 500055,
    };
...
void example_prices() {
    ...
    printEncodingResult("Product Prices", count, encodedSize, meta.encodingType);
...
void example_flags() {
    ...
    uint8_t *encoded = malloc(varintAdaptiveMaxSize(count));
    varintAdaptiveMeta meta;
    size_t encodedSize = varintAdaptiveEncode(encoded, enabledFlags, count, &meta);
...
void example_random() {
    ...
    uint8_t *encoded = malloc(varintAdaptiveMaxSize(count));
    varintAdaptiveMeta meta;
    size_t encodedSize = varintAdaptiveEncode(encoded, randomData, count, &meta);
...
void example_counters() {
    ...
    uint64_t counters[] = {
        1000, 1005, 1012, 1018, 1025, 1033, 1042, 1048,
        1055, 1063, 1071, 1079, 1088, 1095, 1103, 1112,
        1120, 1129, 1137, 1145, 1154, 1162, 1171, 1179,
    };
...
void example_comparison() {
    ...
    const varintAdaptiveEncodingType encodings[] = {
        VARINT_ADAPTIVE_DELTA,
        VARINT_ADAPTIVE_FOR,
        VARINT_ADAPTIVE_PFOR,
        VARINT_ADAPTIVE_DICT,
        VARINT_ADAPTIVE_TAGGED,
    };
    ...
    for (size_t i = 0; i < 5; i++) {
        ...
        printf("  %-10s: %3zu bytes",
               varintAdaptiveEncodingName(encodings[i]), size);
    ...
    printf("\n  Auto-select: %3zu bytes (%s)",
           autoSize, varintAdaptiveEncodingName(meta.encodingType));
...
void example_mixed_patterns() {
    ...
    for (size_t i = 0; i < 20; i++) sequential[i] = 1000 + i;

    /* Pattern 2: Constant */
    uint64_t constant[20];
    for (size_t i = 0; i < 20; i++) constant[i] = 42;

    /* Pattern 3: Powers of 2 */
    uint64_t powers[20];
    for (size_t i = 0; i < 20; i++) powers[i] = 1ULL << i;

    /* Pattern 4: Fibonacci-like */
    uint64_t fib[20];
    fib[0] = 1; fib[1] = 1;
    for (size_t i = 2; i < 20; i++) fib[i] = fib[i-1] + fib[i-2];

    uint8_t *encoded = malloc(varintAdaptiveMaxSize(20));
    varintAdaptiveMeta meta;

    const char *names[] = {"Sequential", "Constant", "Powers-of-2", "Fibonacci"};
