 ...
 * Automatically analyzes ALL structs in the varint library using compiler introspection.
 * This tool uses actual compiler layout information via sizeof() and offsetof().
 *
...
#define ANALYZE_FIELD(struct_type, field_name, field_type, next_offset, struct_size) \
    { #field_name, #field_type, \
      offsetof(struct_type, field_name), \
      sizeof(((struct_type *)0)->field_name), \
      (next_offset) - (offsetof(struct_type, field_name) + sizeof(((struct_type *)0)->field_name)) }

/* Calculate total padding in a struct */
static size_t calculate_padding(const FieldInfo *fields, int field_count, size_t total_size) {
    size_t useful = 0;
...
static void print_field_layout(const char *struct_name, const FieldInfo *fields,
    ...
    for (int i = 0; i < field_count; i++) {
        ...
        printf("  │ %s%2d%s │ %-21s │ %-17s │ %6zu │ %6zu │ %s%7zu%s │\n",
               ...
               fields[i].offset,
               fields[i].size,
               color, fields[i].padding_after, C_RESET);
    ...
    const char *eff_color = efficiency >= 95.0f ? C_GREEN : (efficiency >= 85.0f ? C_YELLOW : C_RED);
    printf("  Total Size: %zu bytes | Useful: %zu bytes | Padding: %s%zu bytes%s | Efficiency: %s%.1f%%%s\n",
           total_size, useful_size, C_YELLOW, total_padding, C_RESET,
           eff_color, efficiency, C_RESET);
}

/* Suggest optimal field ordering */
static void suggest_optimization(const char *struct_name, const FieldInfo *fields,
                                  int field_count, size_t total_size) {
    ...
    for (int i = 0; i < field_count; i++) {
        if (fields[i].padding_after > 0) {
            printf("    • %s%-20s%s (%zu bytes) → %s%zu bytes padding%s\n",
                   C_YELLOW, fields[i].name, C_RESET, fields[i].size,
                   C_RED, fields[i].padding_after, C_RESET);
        }
    }
}
...
static void analyze_varintFORMeta(void) {
    FieldInfo fields[] = {
        ...
        ANALYZE_FIELD(varintFORMeta, range, uint64_t,
                     offsetof(varintFORMeta, offsetWidth), sizeof(varintFORMeta)),
        ...
        ANALYZE_FIELD(varintFORMeta, count, size_t,
                     offsetof(varintFORMeta, encodedSize), sizeof(varintFORMeta)),
        ANALYZE_FIELD(varintFORMeta, encodedSize, size_t,
                     sizeof(varintFORMeta), sizeof(varintFORMeta))
    };
    int field_count = sizeof(fields) / sizeof(fields[0]);

    print_field_layout("varintFORMeta", fields, field_count, sizeof(varintFORMeta));
    suggest_optimization("varintFORMeta", fields, field_count, sizeof(varintFORMeta));
}
...
static void analyze_varintPFORMeta(void) {
    FieldInfo fields[] = {
        ...
        ANALYZE_FIELD(varintPFORMeta, count, uint32_t,
                     offsetof(varintPFORMeta, exceptionCount), sizeof(varintPFORMeta)),
        ANALYZE_FIELD(varintPFORMeta, exceptionCount, uint32_t,
                     offsetof(varintPFORMeta, exceptionMarker), sizeof(varintPFORMeta)),
        ANALYZE_FIELD(varintPFORMeta, exceptionMarker, uint64_t,
                     offsetof(varintPFORMeta, threshold), sizeof(varintPFORMeta)),
        ANALYZE_FIELD(varintPFORMeta, threshold, uint32_t,
                     offsetof(varintPFORMeta, thresholdValue), sizeof(varintPFORMeta)),
        ANALYZE_FIELD(varintPFORMeta, thresholdValue, uint64_t,
                     sizeof(varintPFORMeta), sizeof(varintPFORMeta))
    };
    int field_count = sizeof(fields) / sizeof(fields[0]);

    print_field_layout("varintPFORMeta", fields, field_count, sizeof(varintPFORMeta));
    suggest_optimization("varintPFORMeta", fields, field_count, sizeof(varintPFORMeta));
}
...
static void analyze_varintFloatMeta(void) {
    FieldInfo fields[] = {
        ...
        ANALYZE_FIELD(varintFloatMeta, mode, varintFloatEncodingMode,
                     offsetof(varintFloatMeta, exponentBits), sizeof(varintFloatMeta)),
        ANALYZE_FIELD(varintFloatMeta, exponentBits, uint8_t,
                     offsetof(varintFloatMeta, mantissaBits), sizeof(varintFloatMeta)),
        ANALYZE_FIELD(varintFloatMeta, mantissaBits, uint8_t,
                     offsetof(varintFloatMeta, count), sizeof(varintFloatMeta)),
        ANALYZE_FIELD(varintFloatMeta, count, size_t,
                     offsetof(varintFloatMeta, encodedSize), sizeof(varintFloatMeta)),
        ANALYZE_FIELD(varintFloatMeta, encodedSize, size_t,
                     offsetof(varintFloatMeta, specialCount), sizeof(varintFloatMeta)),
        ANALYZE_FIELD(varintFloatMeta, specialCount, size_t,
                     offsetof(varintFloatMeta, maxRelativeError), sizeof(varintFloatMeta)),
        ANALYZE_FIELD(varintFloatMeta, maxRelativeError, double,
                     sizeof(varintFloatMeta), sizeof(varintFloatMeta))
    };
    int field_count = sizeof(fields) / sizeof(fields[0]);

    print_field_layout("varintFloatMeta", fields, field_count, sizeof(varintFloatMeta));
    suggest_optimization("varintFloatMeta", fields, field_count, sizeof(varintFloatMeta));
}
...
static void analyze_varintAdaptiveDataStats(void) {
    FieldInfo fields[] = {
        ANALYZE_FIELD(varintAdaptiveDataStats, count, size_t,
                     offsetof(varintAdaptiveDataStats, minValue), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, minValue, uint64_t,
                     offsetof(varintAdaptiveDataStats, maxValue), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, maxValue, uint64_t,
                     offsetof(varintAdaptiveDataStats, range), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, range, uint64_t,
                     offsetof(varintAdaptiveDataStats, uniqueCount), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, uniqueCount, size_t,
                     offsetof(varintAdaptiveDataStats, uniqueRatio), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, uniqueRatio, float,
                     offsetof(varintAdaptiveDataStats, isSorted), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, isSorted, bool,
                     offsetof(varintAdaptiveDataStats, isReverseSorted), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, isReverseSorted, bool,
                     offsetof(varintAdaptiveDataStats, avgDelta), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, avgDelta, uint64_t,
                     offsetof(varintAdaptiveDataStats, maxDelta), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, maxDelta, uint64_t,
                     offsetof(varintAdaptiveDataStats, outlierCount), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, outlierCount, size_t,
                     offsetof(varintAdaptiveDataStats, outlierRatio), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, outlierRatio, float,
                     offsetof(varintAdaptiveDataStats, fitsInBitmapRange), sizeof(varintAdaptiveDataStats)),
        ANALYZE_FIELD(varintAdaptiveDataStats, fitsInBitmapRange, bool,
                     sizeof(varintAdaptiveDataStats), sizeof(varintAdaptiveDataStats))
    };
    int field_count = sizeof(fields) / sizeof(fields[0]);

    print_field_layout("varintAdaptiveDataStats", fields, field_count, sizeof(varintAdaptiveDataStats));
    suggest_optimization("varintAdaptiveDataStats", fields, field_count, sizeof(varintAdaptiveDataStats));
}

/* Analyze varintAdaptiveMeta */
static void analyze_varintAdaptiveMeta(void) {
    FieldInfo fields[] = {
        ANALYZE_FIELD(varintAdaptiveMeta, encodingType, varintAdaptiveEncodingType,
                     offsetof(varintAdaptiveMeta, originalCount), sizeof(varintAdaptiveMeta)),
        ANALYZE_FIELD(varintAdaptiveMeta, originalCount, size_t,
                     offsetof(varintAdaptiveMeta, encodedSize), sizeof(varintAdaptiveMeta)),
        ANALYZE_FIELD(varintAdaptiveMeta, encodedSize, size_t,
                     offsetof(varintAdaptiveMeta, encodingMeta), sizeof(varintAdaptiveMeta)),
        { "encodingMeta", "union", offsetof(varintAdaptiveMeta, encodingMeta),
          sizeof(((varintAdaptiveMeta *)0)->encodingMeta),
          sizeof(varintAdaptiveMeta) - offsetof(varintAdaptiveMeta, encodingMeta) -
          sizeof(((varintAdaptiveMeta *)0)->encodingMeta) }
    };
    int field_count = sizeof(fields) / sizeof(fields[0]);

    print_field_layout("varintAdaptiveMeta", fields, field_count, sizeof(varintAdaptiveMeta));
    suggest_optimization("varintAdaptiveMeta", fields, field_count, sizeof(varintAdaptiveMeta));
}
...
static void analyze_varintDictStats(void) {
    FieldInfo fields[] = {
        ANALYZE_FIELD(varintDictStats, uniqueCount, size_t,
                     offsetof(varintDictStats, totalCount), sizeof(varintDictStats)),
        ANALYZE_FIELD(varintDictStats, totalCount, size_t,
                     offsetof(varintDictStats, dictBytes), sizeof(varintDictStats)),
        ANALYZE_FIELD(varintDictStats, dictBytes, size_t,
                     offsetof(varintDictStats, indexBytes), sizeof(varintDictStats)),
        ANALYZE_FIELD(varintDictStats, indexBytes, size_t,
                     offsetof(varintDictStats, totalBytes), sizeof(varintDictStats)),
        ANALYZE_FIELD(varintDictStats, totalBytes, size_t,
                     offsetof(varintDictStats, originalBytes), sizeof(varintDictStats)),
        ANALYZE_FIELD(varintDictStats, originalBytes, size_t,
                     offsetof(varintDictStats, compressionRatio), sizeof(varintDictStats)),
        ANALYZE_FIELD(varintDictStats, compressionRatio, float,
                     offsetof(varintDictStats, spaceReduction), sizeof(varintDictStats)),
        ANALYZE_FIELD(varintDictStats, spaceReduction, float,
                     sizeof(varintDictStats), sizeof(varintDictStats))
    };
    int field_count = sizeof(fields) / sizeof(fields[0]);

    print_field_layout("varintDictStats", fields, field_count, sizeof(varintDictStats));
    suggest_optimization("varintDictStats", fields, field_count, sizeof(varintDictStats));
}
...
static void analyze_varintBitmapStats(void) {
    FieldInfo fields[] = {
        ANALYZE_FIELD(varintBitmapStats, type, varintBitmapContainerType,
                     offsetof(varintBitmapStats, cardinality), sizeof(varintBitmapStats)),
        ANALYZE_FIELD(varintBitmapStats, cardinality, uint32_t,
                     offsetof(varintBitmapStats, sizeBytes), sizeof(varintBitmapStats)),
        ANALYZE_FIELD(varintBitmapStats, sizeBytes, size_t,
                     offsetof(varintBitmapStats, containerCapacity), sizeof(varintBitmapStats)),
        ANALYZE_FIELD(varintBitmapStats, containerCapacity, uint32_t,
                     sizeof(varintBitmapStats), sizeof(varintBitmapStats))
    };
    int field_count = sizeof(fields) / sizeof(fields[0]);

    print_field_layout("varintBitmapStats", fields, field_count, sizeof(varintBitmapStats));
    suggest_optimization("varintBitmapStats", fields, field_count, sizeof(varintBitmapStats));
}
...
static void print_summary(const StructInfo *structs, int count) {
    printf("\n");
    printf("%s╔══════════════════════════════════════════════════════════════════════════════╗%s\n", C_BOLD, C_RESET);
    printf("%s║                    STRUCT MEMORY EFFICIENCY SUMMARY                          ║%s\n", C_BOLD, C_RESET);
    printf("%s╚══════════════════════════════════════════════════════════════════════════════╝%s\n", C_BOLD, C_RESET);
    printf("\n");
    ...
    for (int i = 0; i < count; i++) {
        printf("%-35s %s%8zu%s %8zu    %s%s%s\n",
               structs[i].name,
               C_CYAN, structs[i].size, C_RESET,
               structs[i].align,
               C_BLUE, structs[i].file, C_RESET);
...
int main(void) {
    printf("\n");
    printf("%s╔══════════════════════════════════════════════════════════════════════════════╗%s\n", C_BOLD, C_RESET);
    printf("%s║              Varint Library Comprehensive Struct Memory Audit                ║%s\n", C_BOLD, C_RESET);
    printf("%s║                Using Actual Compiler Layout Information                      ║%s\n", C_BOLD, C_RESET);
    printf("%s╚══════════════════════════════════════════════════════════════════════════════╝%s\n", C_BOLD, C_RESET);
    ...
    printf("\n");
    printf("%s╔══════════════════════════════════════════════════════════════════════════════╗%s\n", C_BOLD, C_RESET);
    printf("%s║                        DETAILED FIELD ANALYSIS                                ║%s\n", C_BOLD, C_RESET);
    printf("%s╚══════════════════════════════════════════════════════════════════════════════╝%s\n", C_BOLD, C_RESET);
    ...
    printf("\n");
    printf("%s╔══════════════════════════════════════════════════════════════════════════════╗%s\n", C_BOLD, C_RESET);
    printf("%s║                            FINAL RECOMMENDATIONS                              ║%s\n", C_BOLD, C_RESET);
    printf("%s╚══════════════════════════════════════════════════════════════════════════════╝%s\n", C_BOLD, C_RESET);
    printf("\n");
    ...
    printf("  • Order fields by alignment: 8-byte → 4-byte → 2-byte → 1-byte\n");
    printf("  • Group same-sized fields together\n");
    printf("  • Use %s_Static_assert%s to prevent regressions\n", C_CYAN, C_RESET);
    printf("  • Consider %s__attribute__((packed))%s only if wire format required\n", C_CYAN, C_RESET);
    printf("\n");
