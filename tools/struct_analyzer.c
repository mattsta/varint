...
static void print_field_layout(const char *struct_name, const char **field_names,
                                size_t *field_offsets, size_t *field_sizes,
                                int field_count, size_t total_size) {
    printf("\n  Field Layout:\n");
    printf("  %-4s %-20s %-8s %-8s %-10s\n", "Idx", "Field", "Offset", "Size", "Padding");
    printf("  %s\n", "------------------------------------------------------------");

    for (int i = 0; i < field_count; i++) {
        size_t next_offset = (i < field_count - 1) ? field_offsets[i + 1] : total_size;
        ...
        printf("  %s%-4d %-20s %-8zu %-8zu %-10zu%s\n",
               color, i, field_names[i], field_offsets[i], field_sizes[i], padding, COLOR_RESET);
    }
}
...
static StructAnalysis analyze_varintFORMeta(void) {
    ...
    const char *fields[] = {"minValue", "maxValue", "range", "offsetWidth", "count", "encodedSize"};
    size_t offsets[] = {
        FIELD_OFFSET(varintFORMeta, minValue),
    ...
    size_t sizes[] = {
        FIELD_SIZE(varintFORMeta, minValue),
    ...
    if (offsets[3] % 8 != 0 && sizes[3] < 8) {
        result.recommendation = "Move offsetWidth to end (1-byte field causes 7 bytes padding)";
    } else {
        result.recommendation = "Optimal";
    }

    print_field_layout(result.name, fields, offsets, sizes, result.fieldCount, result.totalSize);
...
static StructAnalysis analyze_varintPFORMeta(void) {
    ...
    const char *fields[] = {"min", "width", "count", "exceptionCount",
                           "exceptionMarker", "threshold", "thresholdValue"};
    size_t offsets[] = {
        FIELD_OFFSET(varintPFORMeta, min),
    ...
    if (offsets[1] % 8 != 0 && sizes[1] < 8) {
        result.recommendation = "Move width (1-byte) and uint32_t fields to end to reduce padding";
    } else {
        result.recommendation = "Optimal";
    }
...
static StructAnalysis analyze_varintFloatMeta(void) {
    ...
    const char *fields[] = {"precision", "mode", "exponentBits", "mantissaBits",
                           "count", "encodedSize", "specialCount", "maxRelativeError"};
    size_t offsets[] = {
        FIELD_OFFSET(varintFloatMeta, precision),
    ...
    size_t sizes[] = {
        FIELD_SIZE(varintFloatMeta, precision),
    ...
    if (result.paddingSize > 8) {
        result.recommendation = "Move small fields (precision, mode, exponentBits, mantissaBits) to end";
    } else {
        result.recommendation = "Optimal";
    }
...
static StructAnalysis analyze_varintAdaptiveDataStats(void) {
    ...
    const char *fields[] = {"count", "minValue", "maxValue", "range", "uniqueCount",
                           "uniqueRatio", "isSorted", "isReverseSorted", "avgDelta",
                           "maxDelta", "outlierCount", "outlierRatio", "fitsInBitmapRange"};
    size_t offsets[] = {
    ...
    size_t sizes[] = {
        FIELD_SIZE(varintAdaptiveDataStats, count),
    ...
    if (result.paddingSize > 8) {
        result.recommendation = "Move bool fields (isSorted, isReverseSorted, fitsInBitmapRange) to end";
    } else {
        result.recommendation = "Optimal";
    }
...
static StructAnalysis analyze_varintAdaptiveMeta(void) {
    ...
    const char *fields[] = {"encodingType", "originalCount", "encodedSize", "encodingMeta"};
    size_t offsets[] = {
        FIELD_OFFSET(varintAdaptiveMeta, encodingType),
    ...
    size_t sizes[] = {
        FIELD_SIZE(varintAdaptiveMeta, encodingType),
    ...
    if (offsets[0] % 8 != 0 && sizes[0] < 8) {
        result.recommendation = "Move encodingType (enum, 4 bytes) after size_t fields";
    } else {
        result.recommendation = "Optimal";
    }
...
static StructAnalysis analyze_varintDictStats(void) {
    ...
    const char *fields[] = {"uniqueCount", "totalCount", "dictBytes", "indexBytes",
                           "totalBytes", "originalBytes", "compressionRatio", "spaceReduction"};
    size_t offsets[] = {
        FIELD_OFFSET(varintDictStats, uniqueCount),
    ...
    size_t sizes[] = {
        FIELD_SIZE(varintDictStats, uniqueCount),
    ...
    if (result.paddingSize >= 4) {
        result.recommendation = "Move float fields to end together to minimize padding";
    } else {
        result.recommendation = "Optimal";
    }
...
static StructAnalysis analyze_varintBitmapStats(void) {
    ...
    const char *fields[] = {"type", "cardinality", "sizeBytes", "containerCapacity"};
    size_t offsets[] = {
        FIELD_OFFSET(varintBitmapStats, type),
    ...
    size_t sizes[] = {
        FIELD_SIZE(varintBitmapStats, type),
...
static void print_summary(StructAnalysis *results, int count) {
    ...
    printf("%-30s %8s %8s %8s %10s\n", "Struct Name", "Size", "Useful", "Padding", "Efficiency");
    printf("--------------------------------------------------------------------------------\n");
    ...
    for (int i = 0; i < count; i++) {
        ...
        printf("%s%-30s %8zu %8zu %8zu %9.1f%%%s\n",
               color, results[i].name, results[i].totalSize, results[i].usefulSize,
    ...
    printf("--------------------------------------------------------------------------------\n");
    float overall_efficiency = (float)(total_size - total_padding) / total_size * 100.0f;
    ...
    printf("  Structs Needing Optimization: %s%d%s\n",
           inefficient_count > 0 ? COLOR_RED : COLOR_GREEN,
           inefficient_count, COLOR_RESET);
    printf("  Total Memory Wasted: %s%zu bytes%s\n",
           total_padding > 0 ? COLOR_YELLOW : COLOR_GREEN,
           total_padding, COLOR_RESET);
    ...
    for (int i = 0; i < count; i++) {
        if (strcmp(results[i].recommendation, "Optimal") != 0) {
            printf("%s%-30s%s: %s\n", COLOR_YELLOW, results[i].name, COLOR_RESET,
...
int main(void) {
    ...
    printf("\n");
    printf("To apply optimizations, review recommendations and reorder struct fields\n");
    printf("from largest to smallest alignment requirements (8-byte → 4-byte → 2-byte → 1-byte).\n");
    printf("\n");
